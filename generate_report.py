...
def generate_report_for_model(model_type):
    """Generate report for a specific model type."""
    print("="*80)
    print(f"GENERATING REPORT FOR {model_type.upper()} MODEL")
    print("="*80)
    
    # Load models
    logger.info(f"Loading {model_type.upper()} models...")
    clf, autoencoder, ood_detector, ae_threshold, loaded_type = load_models(model_type=model_type)
    
    if clf is None:
        logger.error(f"Failed to load {model_type.upper()} model")
        return False
    
    logger.info(f"✓ {model_type.upper()} models loaded successfully")
    ...
    print(f"Generating markdown report for {model_type.upper()}...")
    
    md_content = []
    md_content.append(f"# MNIST Digit Detection Test Results ({model_type.upper()})")
    md_content.append("")
    md_content.append(f"**Model Type**: {model_type.upper()}")
    ...
    md_content.append("---")
    md_content.append("")
    md_content.append(f"*Report generated by generate_report.py ({model_type.upper()} model)*")
    
    # Write to file with model-specific name
    output_file = f"test_results_report_{model_type}.md"
...
def main():
    ...
    if args.all:
        ...
        if not models_to_process:
            from detection_utils import detect_model_type
            detected = detect_model_type()
            models_to_process = [detected]
    
    print(f"\nGenerating reports for: {', '.join([m.upper() for m in models_to_process])}\n")
    
    results = []
    for model in models_to_process:
        success = generate_report_for_model(model)
