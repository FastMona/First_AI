"""Training program for Neocognitron-inspired classifier only.
...
def main(
    ...
    logger.info("\n🔧 NCT Architecture:")
    logger.info(f"  • Neocognitron-style S/C stages with {Config.NCT_EMBEDDING_SIZE}d embedding")
    logger.info(f"  • Output: {Config.NUM_CLASSES} classes")
    ...
    train_with_early_stopping(
        ...
        num_epochs=epochs,
        patience=3,
        save_path=Config.MODEL_PATH_NCT,
        device=device,
        use_amp=use_amp,
    )

    logger.info("\n" + "=" * 60)
    logger.info("NCT Training Complete")
    logger.info("=" * 60)
    logger.info(f"  - {Config.MODEL_PATH_NCT} (NCT classifier)")
